...
const int switchPin = D2; // ตั้งค่าขาที่เชื่อมต่อสวิตช์ไฟ
const int EEPROM_SIZE = 512; // ขนาด EEPROM
int address = 0; // ตำแหน่งของข้อมูลใน EEPROM
bool isLightOn = false; // สถานะปัจจุบันของไฟ
int lastswitch = 0;
...
void loop() {
  server.handleClient();
if(digitalRead(switchPin) != lastswitch ){
